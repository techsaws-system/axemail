...
import { MASK_SENDER_FROM_EMAIL_EXT, SENDER_TYPE, TEMPLATE_KEY, type MaskSenderFromEmailExt, type SenderType, type TemplateKey } from "@/constants/enums";
import { getSenderCards, sendDomainCampaign, sendGmailCampaign, sendMaskCampaign } from "@/lib/api";
...
export default function TemplateSenderPage() {
  ...
  const [templateKey, setTemplateKey] = useState<TemplateKey>(TEMPLATE_KEY.TEMPLATE_01);
  const [payloadFields, setPayloadFields] = useState<TemplatePayloadFields>(emptyPayloadFields);
  ...
  const activeQuota = senderCardsQuery.data?.find((item) => item.type === senderType);
  const availableTemplates = useMemo(
    () => templateList.filter((template) => template.supportedSenders.includes(senderType)),
    [senderType],
  );
  const activeTemplateKey = availableTemplates.some((template) => template.key === templateKey)
    ? templateKey
    : availableTemplates[0]?.key ?? TEMPLATE_KEY.TEMPLATE_01;
  const activeTemplate = templates[activeTemplateKey];
  const [values, setValues] = useState<Record<string, string>>(() => createEmptyTemplateValues(activeTemplate));
  ...
  const remainingQuota = activeQuota?.remaining ?? Math.max(assignedQuota - usedQuota, 0);
  const noLimitAssigned = assignedQuota === 0;
  const quotaExhausted = !noLimitAssigned && remainingQuota === 0;
  const inputsDisabled = sendMutation.isPending || noLimitAssigned || quotaExhausted;
  ...
  function triggerCooldown() {
    startTransition(() => {
      ...
      const durationSeconds =
        cooldownSchedule[Math.floor(Math.random() * cooldownSchedule.length)] ?? 20;
      ...
      cooldownTimerRef.current = window.setInterval(() => {
        setCooldownSecondsRemaining((current) => {
          ...
          setCooldownProgress(((durationSeconds - clampedValue) / durationSeconds) * 100);
    ...
    <div className="space-y-6">
      {quotaBannerMessage ? (
        <div className="rounded-[1.75rem] border border-amber-200 bg-amber-50 px-5 py-4 shadow-[0_18px_35px_rgba(245,158,11,0.08)]">
          <p className="text-sm font-semibold text-amber-800">{quotaBannerMessage}</p>
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-500">
              Select the sender route, complete the required fields, and dispatch a rendered template.
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Select value={senderType} onValueChange={(value) => handleSenderChange(value as SenderType)}>
            ...
            <Select value={activeTemplateKey} onValueChange={(value) => setTemplateKey(value as TemplateKey)}>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-medium text-slate-900">
                {isDomain ? "Domain cooldown" : isGmail ? "Gmail cooldown" : "Mask cooldown"}
              </p>
            ...
            <div className="text-right">
              <p className="text-[11px] uppercase tracking-[0.22em] text-slate-400">Countdown</p>
              <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-slate-950">
                {cooldownActive ? formatCountdown(cooldownSecondsRemaining) : "Ready"}
              </p>
            </div>
          </div>
      ...
      <div className="dashboard-card">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                The selected template is rendered client-side and submitted through the sender endpoint.
              </p>
            </div>
            {activeQuota ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-right">
                <p className="text-[11px] uppercase tracking-[0.22em] text-slate-400">Remaining quota</p>
                <p className="mt-2 text-lg font-semibold text-slate-950">{activeQuota.remaining}</p>
              </div>
            ) : null}
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Field label="From Name" disabled={inputsDisabled}>
              <Input placeholder="Sender display name" value={payloadFields.fromName} disabled={inputsDisabled} onChange={(event) => setPayloadFields((current) => ({ ...current, fromName: event.target.value }))} />
            </Field>
              ...
              <Field label="From Email" disabled={inputsDisabled}>
                <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2">
                  ...
                  <Select
                    ...
                    disabled={inputsDisabled}
                  >
                    <SelectTrigger className={`w-27.5 ${inputsDisabled ? "border-slate-200 bg-slate-100 text-slate-400" : ""}`}>
            ...
            <Field label="To" disabled={inputsDisabled}>
              <Input placeholder="recipient@company.com" value={payloadFields.to} disabled={inputsDisabled} onChange={(event) => setPayloadFields((current) => ({ ...current, to: event.target.value }))} />
            </Field>

            <Field label="Subject" disabled={inputsDisabled}>
              <Input placeholder="Campaign subject" value={payloadFields.subject} disabled={inputsDisabled} onChange={(event) => setPayloadFields((current) => ({ ...current, subject: event.target.value }))} />
            </Field>

            <Field label="Reply-To" disabled={inputsDisabled}>
              <Input placeholder="reply@company.com" value={payloadFields.replyTo} disabled={inputsDisabled} onChange={(event) => setPayloadFields((current) => ({ ...current, replyTo: event.target.value }))} />
            </Field>

            <Field label="Preview Text" optional disabled={inputsDisabled}>
              <Input placeholder="Inbox preview text" value={payloadFields.previewText} disabled={inputsDisabled} onChange={(event) => setPayloadFields((current) => ({ ...current, previewText: event.target.value }))} />
            </Field>
          </div>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            {activeTemplate.fields.map((field) => (
              <Field key={field.key} label={field.label} optional={field.optional} disabled={inputsDisabled}>
                <Input
          ...
          <div className="mt-6 flex items-center justify-end">
            <Button
              variant="default"
              disabled={cooldownActive || inputsDisabled || !payloadFields.fromName || !payloadFields.to || !payloadFields.replyTo || !payloadFields.subject || (isMask && !payloadFields.fromEmailLocalPart.trim())}
...
function renderTemplate(templateKey: TemplateKey, values: Record<string, string>) {
  switch (templateKey) {
    case TEMPLATE_KEY.TEMPLATE_01:
      return templates[TEMPLATE_KEY.TEMPLATE_01].render(values as Template01HTMLValues);
    case TEMPLATE_KEY.TEMPLATE_02:
      return templates[TEMPLATE_KEY.TEMPLATE_02].render(values as Template02HTMLValues);
    case TEMPLATE_KEY.TEMPLATE_03:
      return templates[TEMPLATE_KEY.TEMPLATE_03].render(values as Template03HTMLValues);
    default:
      return "";
  }
}

function Field({ label, optional, disabled, className, children }: { label: string; optional?: boolean; disabled?: boolean; className?: string; children: ReactNode }) {
  return (
    <div className={className}>
      <div className="flex items-center gap-2">
        <Label className={disabled ? "text-slate-400" : undefined}>{label}</Label>
        {optional ? <span className={`text-[11px] uppercase tracking-[0.22em] ${disabled ? "text-slate-300" : "text-slate-400"}`}>(OPTIONAL)</span> : null}
      </div>
